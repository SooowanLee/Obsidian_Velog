...
1. **SERIALIZABLE**: 트랜잭션을 무조건 **순차적으로 진행**시킨다. 트랜잭션이 끼어들 수 없으니 부정합 문제는 발생하지 않지만, 동시성이 아주 낮아져 그만큼 처리 속도가 느려진다.

2. **REPEATABLE READ**: 
3. **READ UNCOMMITTED(커밋되지 않은 읽기)**: 가장 낮은 수준으로, 트랜잭션이 완료되지 않은 다른 트랜잭션의 데이터를 읽을 수 있습니다.  더티 리드(Dirth Read)가 발생할 수 있습니다.
